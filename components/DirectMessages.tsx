...
function CardChips({ ids }: { ids: string[] }) {
  ...
  return (
    <div className="mt-2 space-y-1">
      {attached.map((record) => (
        <div key={record.id} className="rounded-lg border border-[#e0e0e0] bg-white px-2.5 py-1.5">
          <span className="block text-[11px] leading-snug text-black">{record.title}</span>
          {record.summary && (
            <span className="mt-0.5 block truncate text-[10px] leading-snug text-[#666666]">
              {record.summary}
            </span>
          )}
        </div>
...
function Avatar({ name, size = 40 }: { name: string; size?: number }) {
  return (
    <span
      ...
      className="flex shrink-0 items-center justify-center rounded-full bg-[#f5f5f5] text-black"
    >
        ...
        return (
          <li key={key} className="group relative">
            <button
              type="button"
              onClick={() => onOpen(key)}
              className="flex w-full items-center gap-3 rounded-xl py-2.5 pl-2 pr-16 text-left transition-colors hover:bg-[#f5f5f5]"
            >
            ...
            <span className="absolute right-2 top-1/2 flex -translate-y-1/2 items-center gap-2">
              {pending > 0 && (
                <span
                  title="承認待ち"
                  className="rounded-full bg-black px-1.5 py-0.5 text-[10px] leading-none text-white"
                >
                  {pending}
                </span>
              )}
    ...
    return (
      <div className="mx-auto w-full max-w-[600px] px-5 pt-16 pb-20 lg:pt-10">
        <header className="pb-4">
          <h1 className="text-[17px] font-medium tracking-tight text-black">チャット</h1>
        </header>
    ...
    <div className="mx-auto flex w-full max-w-[600px] flex-col" style={{ height: '100dvh' }}>
      <header className="flex shrink-0 items-center gap-3 border-b border-[#e5e5e5] bg-white px-4 py-3">
        <button
          ...
          className="-ml-1 rounded-lg px-1.5 py-1 text-[18px] leading-none text-black transition-colors hover:bg-[#f5f5f5]"
        >
          ‹
        </button>
        <Avatar name={person.name} size={32} />
        <span className="min-w-0">
          <span className="block truncate text-[14px] leading-tight text-black">{person.name}</span>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
        {thread.length === 0 ? (
          <p className="text-center text-[12px] text-[#666666]">まだやり取りがありません。</p>
        ) : (
          <ul className="space-y-2">
            {thread.map((message, index) => {
              ...
              return (
                <li key={message.id}>
                  {newDay && (
                    <p className="py-3 text-center text-[10px] text-[#666666]">
                  ...
                  <div className={`flex items-end gap-2 ${mine ? 'justify-end' : 'justify-start'}`}>
                    ...
                    <div className="max-w-[78%]">
                      <div
                        className={`rounded-2xl px-3.5 py-2 ${
                          mine
                            ...
                            : 'rounded-bl-md bg-[#f5f5f5] text-black'
                        }`}
                      >
                      ...
                      {message.card && (
                        <div className="mt-1.5">
                          ...
                          {message.cardStatus === 'pending' && (
                            <div className="mt-1.5 rounded-lg border border-[#e5e5e5] bg-white px-2.5 py-2">
                              ...
                              <div className="mt-1.5 flex gap-2">
                                ...
                                <button
                                  ...
                                  className="rounded-full border border-[#e0e0e0] bg-white px-3 py-1.5 text-[11px] text-[#666666] transition-colors hover:border-black hover:text-black"
                                >
                                  今回は見送る
                                </button>
                              </div>
                          ...
                          {message.cardStatus === 'approved' && (
                            <p className="mt-1.5 text-[10px] text-[#666666]">承認されました</p>
                          )}
                          {message.cardStatus === 'declined' && (
                            <p className="mt-1.5 text-[10px] text-[#666666]">見送りになりました</p>
                          )}
                        </div>
                      )}
                      ...
                      <p className={`mt-1 text-[10px] text-[#666666] ${mine ? 'text-right' : ''}`}>
      ...
      <div
        className="shrink-0 border-t border-[#e5e5e5] bg-white px-4 py-3"
      ...
      >
        {locked ? (
          ...
          <form
            ...
            className="flex items-center gap-2 rounded-full border border-[#e0e0e0] bg-[#f5f5f5] px-2 py-1.5 transition-colors focus-within:border-black"
          >
            <input
              value={draft}
              ...
              style={{ fontSize: '16px' }}
              className="min-w-0 flex-1 bg-transparent px-2 text-black placeholder:text-[#666666] focus:outline-none"
            />
            <button
              type="submit"
              disabled={!draft.trim()}
