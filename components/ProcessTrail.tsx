...
export default function ProcessTrail({ steps }: { steps: ProcessStep[] }) {
  ...
  return (
    <div className="rounded-lg border border-line bg-white">
      ...
      >
        <span aria-hidden className={`text-[9px] text-muted ${open ? 'rotate-90' : ''}`}>
          ▶
        </span>
          ...
          {steps.map((step, index) => (
            <li key={index} className="flex gap-3 py-1">
              ...
              <span className="min-w-0">
                <span className="block text-[12px] leading-snug text-ink">{step.label}</span>
                {step.detail && (
                  <span className="block text-[11px] leading-snug text-muted">{step.detail}</span>
                )}
              </span>
            </li>
          ))}
